...
- 2026-06-28：**N5 講稿** `reports/talk_2026-07-03.md`；看板 kanban 轉 N-phase。**N6 起手：多步 route A 已實作**（`sequential_observation.py`：`normalize_base` z-score + `redundancy_mode="maxsim"` MMR；6/6 smoke、合成驗證 λ 掃描成功）。wiki 10 標記 route A done；wiki 11 新增 §9（兩條軸：CL 記憶 vs Agent 多步互不相干 + 7/20 時間軸，便宜記憶建議 LoRA）、§10（λ 白話）。
